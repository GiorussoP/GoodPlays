...
def test_create_review_success(client):
    ...
    user_response = client.post("/users/", json={"username": "testuser", "email": "test@test.com", "password": "pass123"})
    game_response = client.post("/games/", json={"title": "Test Game", "developer": "Test Dev"})
...
def test_get_review_success(client):
    ...
    user_response = client.post("/users/", json={"username": "testuser2", "email": "test2@test.com", "password": "pass123"})
    game_response = client.post("/games/", json={"title": "Test Game 2", "developer": "Test Dev 2"})
...
def test_update_review_success(client):
    # Criar usuário, jogo e review
    user_response = client.post("/users/", json={"username": "testuser3", "email": "test3@test.com", "password": "pass123"})
    game_response = client.post("/games/", json={"title": "Test Game 3", "developer": "Test Dev 3"})
...
def test_delete_review_success(client):
    # Criar usuário, jogo e review
    user_response = client.post("/users/", json={"username": "testuser4", "email": "test4@test.com", "password": "pass123"})
    game_response = client.post("/games/", json={"title": "Test Game 4", "developer": "Test Dev 4"})
...
def test_get_game_reviews_success(client):
    # Criar usuário, jogo e review
    user_response = client.post("/users/", json={"username": "testuser5", "email": "test5@test.com", "password": "pass123"})
    game_response = client.post("/games/", json={"title": "Test Game 5", "developer": "Test Dev 5"})
    
    user_id = user_response.json()["id"]
    game_id = game_response.json()["id"]
    
    # Criar múltiplas reviews para o jogo
    review1_response = client.post("/reviews/", json={
        "user_id": user_id,
        "game_id": game_id,
        "rating": 5,
        "comment": "Excelente jogo!"
    ...
    review2_response = client.post("/reviews/", json={
        "user_id": user_id,
        "game_id": game_id,
        "rating": 4,
        "comment": "Bom jogo"
    ...
    response = client.get(f"/reviews/game/{game_id}")
    assert response.status_code == 200
    data = response.json()
...
def test_get_game_reviews_empty(client):
...
def test_update_review_invalid_rating(client):
    # Criar usuário, jogo e review
    user_response = client.post("/users/", json={"username": "testuser6", "email": "test6@test.com", "password": "pass123"})
    game_response = client.post("/games/", json={"title": "Test Game 6", "developer": "Test Dev 6"})
